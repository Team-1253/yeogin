...
### 5.6 pytest 밖 상호작용 시나리오 검증

- 12종 멀티턴 시나리오(확인·부정·주제전환·후보명·번형·범위밖·단독숫자·조건누적·
  인젝션·공백특수·되묻기무시·장소변경) 실행 — 크래시·스택 누출 없음.
- 실이슈 1건: "홍대입구역으로 바꿔줘"가 장소 추출 실패. 원인 — 접미사 정규식이
  "홍대입구역으로"를 통째로 매칭한 뒤 조사 가드가 통째로 버려 짧은 후보를
  재시도하지 않음. 조치 — `_strip_trailing_josa`로 조사 벗긴 뒤 접미사 재확인
  ("2시간으로"는 여전히 제외, "역삼로" 보존). 주제전환 정상화 실증.
