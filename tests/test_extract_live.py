...
CI나 평소 검증에 넣지 않습니다. 슬롯 분리 후 호출 수가 늘었습니다.
(`_extract_by_llm` 1회 = 슬롯 4회, 재시도 시 더 늘어납니다.)
...
def test_live_S1을_파싱한다():
    """S1 발화를 실제 모델로 파싱합니다. (API 3회 호출: sort 생략)"""
...
def test_live_빈슬롯은_None이다(monkeypatch):
    """장소만 있는 발화는 나머지 슬롯이 비어 있습니다. (API 4회 호출)"""
    monkeypatch.setenv("PARKING_AGENT_NO_LLM", "0")
